...
function updateAdminView(){
  ...
  <div class="container" >
    <div  class="alert-message">
      ${model.inputs.adminPage.isSubmitted && !checkAllRequiredFields(model.inputs.adminPage.happening) 
         ? createAlertMessage("error","Du bør sjekke inn på noen av disse feltene nedenfor.") 
         : ""}
      ${model.inputs.adminPage.isSubmitted && checkAllRequiredFields(model.inputs.adminPage.happening) 
        ? createAlertMessage("success","Innsendingproesess er vellykket") 
        : ""}
    </div>
    <div class="header">
...
<div class="happeningformRow happeningTitle">
   <label for="title" class="formLabel">
      ...
      <span class="titleText">Tittel på happening</span>
   </label>
   <input
     autofocus
     class="form-element input-top-mrgn
      ...
      ${model.inputs.adminPage.isSubmitted 
        ? showValidationMessage(model.inputs.adminPage.happening.title.isValidate,"Tittel på happening påkrevd") 
        : ""}
 </div>
`;
...
happeningMainCategory+=`
  <div class="happeningformRow happeningMainCategory">
    <label for="mainCategory" class="formLabel ">
      <span class="requiredStar">*</span>
      <span class="tittelText">Velg kategori</span>
    </label>
    <select    class="form-element mainCategory  input-top-mrgn
      ...
      onchange="model.inputs.adminPage.happening.categoryId.name=this.value; updateView()"
    >
    <option selected disabled hidden>${model.inputs.adminPage.happening.categoryId.name 
...
for(let i=0; i<categories.length; i++){
  ...
  happeningMainCategory+=`
    ...
    ${model.inputs.adminPage.isSubmitted 
      ? showValidationMessage(model.inputs.adminPage.happening.categoryId.isValidate,"Kategorifelt er påkrevd") 
      : ""}
</div>`;
...
function createPaymentCategoryHtml(){
  ...
  <div class="happeningformRow ">
    <label for="paymentCategory" class="formLabel ">
      <span class="requiredStar">*</span>
      <span class="tittelText">Velg payment type</span>
    </label>
    <div class="paymentCategory  input-top-mrgn ">
      <div class="paymentTypeContainer  form-element 
      ${model.inputs.adminPage.isSubmitted 
        ? showValidationStyle(model.inputs.adminPage.happening.paymentTypeId.isValidate,"errorStyle","successStyle")
        : "" }
      ">
      `;
      ...
      <div class="paymentType">
        <input
          ...
          type="radio" name="radio" class="payTypeInput"
          id="${paymentType.id}" 
  ...
  ${model.inputs.adminPage.isSubmitted 
    ? showValidationMessage(model.inputs.adminPage.happening.paymentTypeId.isValidate,"Paymenttypefelt er påkrevd") 
    : ""}  
  </div>
  `;
...
function createHappeningDate(){
  ...
  <label  class="formLabel">
    <span class="requiredStar">*</span>
    <span >Happening start-end dato og klokkaslett
    </span>
  </label>
  ...
  ${model.inputs.adminPage.isSubmitted 
    ? showValidationStyle(model.inputs.adminPage.happening.happeningStartDate.isValidate,"errorStyle","successStyle")
    : "" }"
   id="happeningStart"  type="datetime-local"

    value="${this.value=model.inputs.adminPage.happening.happeningStartDate.name 
      ? (model.inputs.adminPage.happening.happeningStartDate.name+"T"+ model.inputs.adminPage.happening.happeningStartTime.name) 
      :  now.toISOString().slice(0, 16)}"
    onchange="getHappeningStartDate(this);"
    />
    ${model.inputs.adminPage.isSubmitted 
      ? showValidationMessage(model.inputs.adminPage.happening.happeningStartDate.isValidate,"happeningStartDate er påkrevd") 
      : ""}
  </div>
      <span class="spantext date-margin">til</span> 
      <div class="happeningDateSection">
        <input
          min="${now.toISOString().slice(0, 16)}"
          class="form-date happeningDateInput
          ${model.inputs.adminPage.isSubmitted 
            ? showValidationStyle(model.inputs.adminPage.happening.happeningEndDate.isValidate,"errorStyle","successStyle")
            : "" }
          " id="happeningEnd"  type="datetime-local"
          value="${this.value=model.inputs.adminPage.happening.happeningEndDate.name 
            ? (model.inputs.adminPage.happening.happeningEndDate.name +"T"+ model.inputs.adminPage.happening.happeningEndTime.name) 
            : now.toISOString().slice(0, 16)}"
          onchange="getHappeningEndDate(this)"
        />   
       ${model.inputs.adminPage.isSubmitted 
        ? showValidationMessage(model.inputs.adminPage.happening.happeningEndDate.isValidate,"happeningEndDate er påkrevd") 
        : ""}
  </div> 
 </div>
`;
...
function createAnnouncementDate(){
  ...
  announcementDate+=`
  <label  class="formLabelAnnounce"> 
    <span >Annonsering start-end dato og klokkaslett
    </span>
  </label>
